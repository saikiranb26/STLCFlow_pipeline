...
exports.buildStoryAnalysisBundle = buildStoryAnalysisBundle;
exports.buildStoryAnalysisBundleFromStory = buildStoryAnalysisBundleFromStory;
...
const LEGACY_REPORT_REFERENCE_SUITES = [
    {
        planId: 191930,
        suiteId: 195328,
...
function inferFeatureArea(storyHints) {
...
function getCoverageFamilies(domain, featureArea) {
...
function getOpenQuestionPrompts(domain, featureArea) {
    if (featureArea === "Legacy Reports") {
        return [
...
function matchesDomain(domain, suiteDomain) {
    if (domain === "TDL") {
...
function getPriorityBias(domain, suiteId) {
...
function buildSelectionReason(domain, suite) {
    if (domain === "TDL") {
...
function buildStoryAnalysisBundle(context) {
    const rootsDoc = (0, knowledge_1.loadReferenceRoots)(context.knowledgeRoot);
    const corpusDoc = (0, knowledge_1.loadReferenceSuiteCorpus)(context.knowledgeRoot);
    const domain = inferDomain(context);
    const liveEvidenceMode = context.input.navigationPath && context.input.navigationPath.trim()
        ? "playwright-eligible"
        : "requirement-first";
...
function buildStoryAnalysisBundleFromStory(context, storyHints) {
