...
exports.generateWorkbookFromStoryArtifacts = generateWorkbookFromStoryArtifacts;
...
function sanitizeWorkbookTitle(value) {
    return clean(value).replace(/[<>:"/\\|?*\x00-\x1F]/g, " ").trim();
}
function splitRequirementItems(rawValue) {
...
function buildCaseTitlePrefix(story) {
...
function inferNavigationStep(story) {
...
function summarizeRequirement(value) {
...
function normalizeWorkbookPriority(value) {
...
function parseAcceptanceScenarios(value) {
...
function buildLegacyReportsCases(story) {
...
function buildHappyPathSteps(story, requirement) {
    return [
        { action: "Login to Match.", expected: "User is logged in successfully." },
...
function buildValidationCase(story, requirement) {
    return {
...
function buildListViewCase(story, coveragePlan) {
    ...
    const shouldCreate = families.some((family) => family.toLowerCase().includes("list-view")) ||
...
function buildCancelCase(story) {
    return {
...
function buildChildTaskCase(story) {
    return {
...
function buildDraftCases(story, coveragePlan) {
    ...
    const requirementItems = Array.from(new Set([
    ...
    if (generatedCases.length === 0) {
        generatedCases.push(buildValidationCase(story, story.description || "Current story behavior requires clarification from the work item."));
    }
...
function buildTimestampedWorkbookPath(workbookPath) {
    const extension = node_path_1.default.extname(workbookPath) || ".xlsx";
    const baseName = node_path_1.default.basename(workbookPath, extension);
    const directory = node_path_1.default.dirname(workbookPath);
    return node_path_1.default.join(directory, `${baseName} ${Date.now()}${extension}`);
}
function writeWorkbookFile(templatePath, workbookPath, story, cases) {
    ...
    (0, fs_1.ensureDir)(node_path_1.default.dirname(workbookPath));
    try {
...
function generateWorkbookFromStoryArtifacts(context, templatePath, story, coveragePlan) {
    const requestedWorkbookPath = (0, workbook_conventions_1.getExpectedWorkbookPath)(context.projectRoot, context.input);
