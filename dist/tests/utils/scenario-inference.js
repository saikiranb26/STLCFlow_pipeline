...
exports.inferExecutionStepsFromManualSteps = inferExecutionStepsFromManualSteps;
exports.isUiScenario = isUiScenario;
...
function splitCompoundAction(action) {
...
function parseExampleValue(value) {
    const match = value.match(/example\s*:\s*(.+)$/i);
    return match?.[1] ? clean(match[1]) : undefined;
}
function buildCustomStep(notes, expected) {
    return {
        kind: "custom",
        notes: clean([notes, expected].filter(Boolean).join(" | "))
    };
}
function buildAssertVisibleSteps(values) {
...
function splitExpectedAssertions(value) {
...
function inferStructuredManualStep(manualStep, navigationPath) {
    ...
    const pageSizeMatch = action.match(/^change the page size in legacy reports list\.?\s*example:\s*(\d+)/i) ||
    ...
    if (/^verify the legacy reports list remains on page \d+ after the move action\.?$/i.test(action) ||
    ...
    if (/enter .*task name.*select .*import.*click .*create task/i.test(action) ||
    ...
    if (/^select "dependent upon the completion of another task"/i.test(action)) {
        return [
            { kind: "clickText", target: "Dependent upon the completion of another task" },
    ...
    const importTypeSelectionMatch = action.match(/^select\s+(bai import|standard(?: import)?|extended standard(?: import)?|legacy custom import|custom import)\b/i);
...
function inferStepFromSegment(segment, expected, navigationPath) {
    ...
    const selectDropdownMatch = normalizedBase.match(/^select\s+(.+?)\s+(?:from|in)\s+(.+?)\s+dropdown$/i) ||
    ...
    const recurrenceTypeMatch = normalizedBase.match(/select\s+(.+?)\s+radio button\s+for\s+(.+)$/i) ||
        normalizedBase.match(/select\s+["']?([^"']+)["']?\s+for\s+(.+)$/i);
    ...
    if (quotedSelect?.[1]) {
        return {
            kind: "clickText",
            target: stripTrailingPunctuation(quotedSelect[1])
        };
    }
...
function inferExecutionStepsFromManualSteps(input) {
    const steps = [];
...
function isUiScenario(executionSteps) {
